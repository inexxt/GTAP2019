...
diamond .(_ ∷ _ ∷ r) m2 m3 (swap≅ x [] r .(_ ∷ _ ∷ r) .m2 refl defmf) (swap≅ x₁ (x₂ ∷ x₃ ∷ l) r₁ .(_ ∷ _ ∷ r) .m3 d defmf₁)
    rewrite defmf rewrite defmf₁ rewrite (≡-sym (cut-t1 d)) rewrite (≡-sym (cut-t2 d)) rewrite (cut-h2 d) =
     _ , (swap x₁ (_ ∷ _ ∷ l) r₁ , swap x [] (l ++ _ ∷ _ ∷ r₁)) -- ss-dis
...
diamond (x1 ∷ m1) (x2 ∷ m2) (x3 ∷ m3) (cancel≅ (x ∷ l) r .(x1 ∷ m1) .(x2 ∷ m2) defm defmf) (cancel≅ (x₁ ∷ l₁) r₁ .(x1 ∷ m1) .(x3 ∷ m3) defm₁ defmf₁)  =
  ...
  in  _ , (l++ [ x2 ] rec-l , trans (refl≡ (head+tail (≡-trans (cut-tail defmf₁) (≡-trans (cut-tail lemma) (cut-tail (≡-sym defmf)))) refl)) (l++ [ x2 ] rec-r))
