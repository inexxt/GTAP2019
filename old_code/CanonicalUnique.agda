...
open ≤-Reasoning renaming (begin_ to ≤-begin_)

open import Coxeter hiding (n; l; _≅_)
open ≃-Reasoning
...
abs-const-↓ n (suc (suc k)) a [] r p with ≡-trans (cut-t1 p) (≡-sym (cut-t2 p))
abs-const-↓ n (suc (suc k)) a [] r p | x = abs-≡ x
abs-const-↓ n (suc k) a (x ∷ l) r p = abs-const-↓ n k a l r (cut-head p)

abs-2≡ : {n : ℕ} -> suc (suc n) ≡ n -> ⊥
abs-2≡ ()

abs-inc-↓ : (n k a : ℕ) -> (l r : List ℕ) -> (n ↓ k) ≡ (l ++ a ∷ suc a ∷ r) -> ⊥
abs-inc-↓ n (suc (suc k)) a [] r p with ≡-trans (cong suc (cut-t1 p)) (≡-sym (cut-t2 p))
abs-inc-↓ n (suc (suc k)) a [] r p | x = abs-2≡ x
abs-inc-↓ n (suc k) a (x ∷ l) r p = abs-inc-↓ _ _ _ _ _ (cut-head p)

abs-jump-↓ : (n k a b : ℕ) -> (suc a < b) -> (l r : List ℕ) -> (n ↓ k) ≡ (l ++ b ∷ a ∷ r) -> ⊥
abs-jump-↓ n (suc (suc k)) a b x [] r p =
  let p1 = cut-t1 p
      p2 = cut-t2 p
      t1 =
        ≤-begin
          suc (suc (k + n))
        ≡⟨ cong suc (cong suc p2) ⟩ 
          suc (suc a)
        ≤⟨ x ⟩
          b
        ≡⟨ ≡-sym p1  ⟩ 
        ...
        ∎
  in  abs-refl t1
abs-jump-↓ n (suc k) a b x (e ∷ l) r p = abs-jump-↓ n k a b x l r (cut-head p)
