...
open ≤-Reasoning renaming (_∎ to _≤∎; begin_ to begin-≤_) hiding (step-≡)
...
canonize-swap (suc n) (suc r) i prn pni =
  ...
  in  ≃begin
         i ∷ n ∷ (n ↓ r)
       ≃⟨ ++-respects-l (swap pni) ⟩  -- ++-respects-r {l = [ n ]} rec ⟩
         n ∷ i ∷ (n ↓ r)
       ≃⟨ ++-respects-r {l = [ n ]} rec ⟩
          n ∷ (n ↓ r) ++ i ∷ []
       ≃∎
...
canonize-p>' (suc (suc n)) zero (suc r2) {i} pr2 prn {pinr} rewrite pinr = -- induction on r2
  ...
  in  ≃begin
         (1 + n) ∷ n ∷ (n ↓ r2) ++ (1 + n) ∷ []
       ≃⟨ ++-respects-r {l = (1 + n) ∷ [ n ]} (comm rec) ⟩
         (1 + n) ∷ n ∷ (1 + n) ∷ (n ↓ r2)
       ≃⟨ ++-respects-l braid ⟩ --  ++-respects-l (comm braid)  ⟩
         n ∷ suc n ∷ n ∷ (n ↓ r2)
       ≃∎
